...
trait FiatHostedChannelMessage extends LightningMessage

case class InvokeFiatHostedChannel(chainHash: ByteVector32, refundScriptPubKey: ByteVector, secret: ByteVector = ByteVector.empty) extends FiatHostedChannelMessage

case class InitFiatHostedChannel(maxHtlcValueInFlightMsat: UInt64, htlcMinimumMsat: MilliSatoshi, maxAcceptedHtlcs: Int, channelCapacityMsat: MilliSatoshi,
                             initialClientBalanceMsat: MilliSatoshi, features: List[Int] = Nil) extends FiatHostedChannelMessage

case class FiatHostedChannelBranding(rgbColor: Color, pngIcon: Option[ByteVector], contactInfo: String) extends FiatHostedChannelMessage

case class FiatLastCrossSignedState(isHost: Boolean, refundScriptPubKey: ByteVector, initHostedChannel: InitFiatHostedChannel, blockDay: Long, localBalanceMsat: MilliSatoshi,
                                remoteBalanceMsat: MilliSatoshi, localUpdates: Long, remoteUpdates: Long, incomingHtlcs: List[UpdateAddHtlc], outgoingHtlcs: List[UpdateAddHtlc],
                                remoteSigOfLocal: ByteVector64, localSigOfRemote: ByteVector64) extends FiatHostedChannelMessage {

  lazy val reverse: FiatLastCrossSignedState =
    copy(isHost = !isHost, localUpdates = remoteUpdates, remoteUpdates = localUpdates,
      localBalanceMsat = remoteBalanceMsat, remoteBalanceMsat = localBalanceMsat,
      remoteSigOfLocal = localSigOfRemote, localSigOfRemote = remoteSigOfLocal,
      incomingHtlcs = outgoingHtlcs, outgoingHtlcs = incomingHtlcs)
  ...
  def withLocalSigOfRemote(priv: PrivateKey): FiatLastCrossSignedState = {
    val localSignature = Crypto.sign(reverse.hostedSigHash, priv)
    copy(localSigOfRemote = localSignature)
  }
}

case class FiatStateUpdate(blockDay: Long, localUpdates: Long, remoteUpdates: Long, localSigOfRemoteLCSS: ByteVector64) extends FiatHostedChannelMessage

case class FiatStateOverride(blockDay: Long, localBalanceMsat: MilliSatoshi, localUpdates: Long, remoteUpdates: Long, localSigOfRemoteLCSS: ByteVector64) extends FiatHostedChannelMessage

case class FiatAnnouncementSignature(nodeSignature: ByteVector64, wantsReply: Boolean) extends FiatHostedChannelMessage

case class FiatResizeChannel(newCapacity: Satoshi, clientSig: ByteVector64 = ByteVector64.Zeroes) extends FiatHostedChannelMessage {
  def isRemoteResized(remote: FiatLastCrossSignedState): Boolean = newCapacity.toMilliSatoshi == remote.initHostedChannel.channelCapacityMsat
  def sign(priv: PrivateKey): FiatResizeChannel = FiatResizeChannel(clientSig = Crypto.sign(Crypto.sha256(sigMaterial), priv), newCapacity = newCapacity)
  def verifyClientSig(pubKey: PublicKey): Boolean = Crypto.verifySignature(Crypto.sha256(sigMaterial), clientSig, pubKey)
  lazy val sigMaterial: ByteVector = Protocol.writeUInt64(newCapacity.toLong, ByteOrder.LITTLE_ENDIAN)
  lazy val newCapacityMsatU64: UInt64 = UInt64(newCapacity.toMilliSatoshi.toLong)
}

case class FiatAskBrandingInfo(chainHash: ByteVector32) extends FiatHostedChannelMessage

// PHC

case class QueryPublicFiatHostedChannels(chainHash: ByteVector32) extends FiatHostedChannelMessage

case class ReplyPublicFiatHostedChannelsEnd(chainHash: ByteVector32) extends FiatHostedChannelMessage

// Preimage queries

case class FiatQueryPreimages(hashes: List[ByteVector32] = Nil) extends FiatHostedChannelMessage

case class FiatReplyPreimages(preimages: List[ByteVector32] = Nil) extends FiatHostedChannelMessage
