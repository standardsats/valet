...
class DBInterfaceSQLiteAndroidGraph(context: Context, name: String) extends SQLiteOpenHelper(context, name, null, 1) with DBInterfaceSQLiteAndroid {
  ...
  def onCreate(dbs: SQLiteDatabase): Unit = {
    ...
    FiatHostedChannelAnnouncementTable.createStatements.foreach(dbs.execSQL)
    ...
    FiatHostedExcludedChannelTable.createStatements.foreach(dbs.execSQL)
    ...
    FiatHostedChannelUpdateTable.createStatements.foreach(dbs.execSQL)
  }
