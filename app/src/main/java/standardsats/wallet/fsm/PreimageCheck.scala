package standardsats.wallet.fsm

import java.util.concurrent.Executors
import fr.acinq.bitcoin.{ByteVector32, Crypto}
import fr.acinq.eclair.channel.Helpers.HashToPreimage
import fr.acinq.eclair.wire.{FiatHostedChannelMessage, FiatQueryPreimages, FiatReplyPreimages, Init, QueryPreimages, ReplyPreimages}
...
object PreimageCheck {
  ...
  case class PeerResponse(msg: FiatHostedChannelMessage, worker: CommsTower.Worker)
...
abstract class PreimageCheck extends StateMachine[PreimageCheck.CheckData] { me =>
  ...
  private lazy val listener = new ConnectionListener {
    ...
    override def onOperational(worker: CommsTower.Worker, theirInit: Init): Unit = worker.handler process FiatQueryPreimages(data.hashes.toList)
    override def onFiatHostedMessage(worker: CommsTower.Worker, msg: FiatHostedChannelMessage): Unit = me process PreimageCheck.PeerResponse(msg, worker)
  }
  ...
  def doProcess(change: Any): Unit = (change, state) match {
    ...
    case (PreimageCheck.PeerResponse(msg: FiatReplyPreimages, worker), OPERATIONAL) =>
  ...
  def merge(data1: PreimageCheck.CheckData, msg: FiatReplyPreimages): PreimageCheck.CheckData = {
